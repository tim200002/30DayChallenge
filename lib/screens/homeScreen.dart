...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: BlocBuilder(
        bloc: BlocProvider.of<BlocHomeScreen>(context),
        builder: (context, MainScreenState state) {
          ...
          if (state is MainUninitialized) {
            BlocProvider.of<BlocHomeScreen>(context)
                .dispatch(FetchAll()); //! This was changed
            return SafeArea(
              child: FlatButton(
                color: Colors.blue,
                child: Text("Hello"),
          ...
          } else if (state is ShowLoginScreeen) {
            log("state");
            return LoginScreen2();
          }

          //Display all Activities on the Main Screen
